...
from src.core.models import (CircuitBreakerLevel, Order, OrderSide, Portfolio,
...
class RiskManager:
    ...
    def reset_periods(self, portfolio: Portfolio):
        ...
        if self.last_reset_day:
            ...
            if now.date() != last_reset.date():
                ...
                logger.info(
                    "risk_manager.daily_reset", new_balance=str(portfolio.total_balance)
                )

        # Reset weekly
        if self.last_reset_week:
            # Handle both timezone-aware and naive datetimes for compatibility
            last_reset = self.last_reset_week
            if last_reset.tzinfo is None:
                last_reset = last_reset.replace(tzinfo=timezone.utc)
            ...
            if days_since_reset >= 7:
                ...
                self.last_reset_week = now
                logger.info(
                    "risk_manager.weekly_reset",
                    new_balance=str(portfolio.total_balance),
                )
    ...
    def check_signal(
        ...
        if (
            self.all_time_high_balance
            and portfolio.total_balance > self.all_time_high_balance
        ):
            self.all_time_high_balance = portfolio.total_balance
    ...
    def _check_emergency_stop(
        ...
        if self.emergency_stop:
            return RiskCheck(
                ...
                metadata={
                    "triggered_at": (
                        self.emergency_triggered_at.isoformat()
    ...
    def _check_circuit_breaker_rule(
        ...
        if cb.level == CircuitBreakerLevel.LEVEL_4:
            return RiskCheck(
                ...
                metadata={
                    ...
                    "triggered_at": (
                        cb.triggered_at.isoformat() if cb.triggered_at else None
                    ),
                },
            )

        if (
            cb.pause_new_entries
            and cb.pause_until
            and datetime.now(timezone.utc) < cb.pause_until
        ):
            return RiskCheck(
                passed=False,
        ...
        if cb.close_directional and signal.signal_type in (
            SignalType.BUY,
            SignalType.SELL,
        ):
            return RiskCheck(
    ...
    def _check_weekly_loss_limit(
        ...
        if weekly_loss_pct >= max_weekly_loss_pct:
            self.trigger_emergency_stop(
                f"Weekly loss limit reached: {weekly_loss_pct:.2f}%"
            )
            return RiskCheck(
                passed=False,
                ...
                metadata={
                    ...
                    "limit": float(max_weekly_loss_pct),
                },
            )
    ...
    def _check_max_position_size(
        ...
        if signal.symbol in current_positions:
            ...
            portfolio_pct = (
                position_value / portfolio.total_balance * 100
            ...
            if portfolio_pct >= max_pct:
                return RiskCheck(
                    ...
                    risk_level="critical",
                    metadata={
                        "current_pct": float(portfolio_pct),
    ...
    def _check_max_concurrent_positions(
        ...
        open_positions = len(
            [p for p in current_positions.values() if p.side != PositionSide.NONE]
        )
        ...
        if avg_correlation >= self.CORRELATION_CRISIS_THRESHOLD:
            return RiskCheck(
                ...
                metadata={
                    ...
                    "threshold": float(self.CORRELATION_CRISIS_THRESHOLD),
                },
            )
        ...
        if confidence < self.MIN_SIGNAL_CONFIDENCE:
            return RiskCheck(
                ...
                metadata={
                    ...
                    "minimum": float(self.MIN_SIGNAL_CONFIDENCE),
                },
            )
    ...
    def _check_duplicate_position(
        ...
        Blocks duplicate positions for risk management. Buy signals are rejected
        if a LONG position already exists, and Sell signals are rejected if a
        SHORT position already exists.
        """
        if signal.symbol in current_positions:
            existing = current_positions[signal.symbol]

            # Block duplicate LONG positions (BUY signal with existing LONG)
            if (
    ...
    def check_circuit_breakers(self, portfolio: Portfolio) -> CircuitBreakerLevel:
        ...
        for level, threshold in sorted(
            self.CIRCUIT_BREAKER_LEVELS.items(), key=lambda x: x[1], reverse=True
        ):
        ...
        if self.circuit_breaker.level == CircuitBreakerLevel.LEVEL_1:
            recovery_threshold = self.CIRCUIT_BREAKER_ACTIONS[
                CircuitBreakerLevel.LEVEL_1
            ]["auto_recovery_drawdown"]
    ...
    def _activate_circuit_breaker(
        self, level: CircuitBreakerLevel, drawdown: Decimal, portfolio: Portfolio
    ):
        ...
        # Apply actions
        self.circuit_breaker.reduce_positions_pct = actions.get(
            "reduce_positions_pct", Decimal("0")
        )
        ...
        self.circuit_breaker.widen_stops_pct = actions.get(
            "widen_stops_pct", Decimal("0")
        )

        # Set pause duration if applicable
        if "pause_duration_hours" in actions:
            self.circuit_breaker.pause_until = now + timedelta(
                hours=actions["pause_duration_hours"]
            )
        ...
        logger.critical(
            ...
            drawdown=float(drawdown),
            actions={
                k: str(v) if isinstance(v, Decimal) else v for k, v in actions.items()
            },
            portfolio_value=str(portfolio.total_balance),
        )
    ...
    def reset_circuit_breaker(self) -> bool:
        ...
        if actions.get("manual_recovery_required", False) or actions.get(
    ...
    def calculate_position_size(
        ...
        risk_per_trade = (
            risk_pct if risk_pct is not None else self.DEFAULT_RISK_PER_TRADE
        )
        ...
        position_value = min(
            position_value, portfolio.available_balance * Decimal("0.95")
        )
    ...
    def _calculate_kelly_position_size(
        self, portfolio: Portfolio, win_rate: Decimal, avg_win_loss_ratio: Decimal
    ) -> Decimal:
        """
        ...
        if win_rate <= 0 or avg_win_loss_ratio <= 0:
            return (
                ...
                * Decimal(str(trading_config.max_position_pct))
                / 100
            )
