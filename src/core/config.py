...
class CapitalAllocationConfig(BaseSettings):
    ...
    @field_validator("allocation_core_hodl", "allocation_trend", 
                     "allocation_funding", "allocation_tactical")
    @classmethod
    ...
    def total_allocation(self) -> float:
        ...
        return (self.allocation_core_hodl + self.allocation_trend + 
                self.allocation_funding + self.allocation_tactical)
...
class CircuitBreakerConfig(BaseSettings):
    ...
    level_1_action: str = Field(default="reduce_position_size", env="CIRCUIT_BREAKER_1_ACTION")
    ...
    level_2_action: str = Field(default="reduce_and_pause", env="CIRCUIT_BREAKER_2_ACTION")
    ...
    level_3_action: str = Field(default="close_directional", env="CIRCUIT_BREAKER_3_ACTION")
    level_3_halt_trading: bool = Field(default=True, env="CIRCUIT_BREAKER_3_HALT_TRADING")
    
    # Level 4: EMERGENCY (25% drawdown)
    level_4_threshold: float = Field(default=0.25, env="CIRCUIT_BREAKER_4_THRESHOLD")
    level_4_action: str = Field(default="emergency_liquidation", env="CIRCUIT_BREAKER_4_ACTION")
    level_4_halt_trading: bool = Field(default=True, env="CIRCUIT_BREAKER_4_HALT_TRADING")
    
    @field_validator("level_1_threshold", "level_2_threshold", 
                     "level_3_threshold", "level_4_threshold")
    @classmethod
...
class TacticalConfig(BaseSettings):
    ...
    trigger_10_pct_allocation: float = Field(default=0.10, env="TACTICAL_TRIGGER_10_PCT_ALLOCATION")
    trigger_20_pct_allocation: float = Field(default=0.15, env="TACTICAL_TRIGGER_20_PCT_ALLOCATION")
    trigger_30_pct_allocation: float = Field(default=0.20, env="TACTICAL_TRIGGER_30_PCT_ALLOCATION")
    trigger_40_pct_allocation: float = Field(default=0.25, env="TACTICAL_TRIGGER_40_PCT_ALLOCATION")
    trigger_50_pct_allocation: float = Field(default=0.30, env="TACTICAL_TRIGGER_50_PCT_ALLOCATION")
    
    # Fear & Greed thresholds
    fear_greed_extreme_fear: int = Field(default=20, env="TACTICAL_FEAR_GREED_EXTREME_FEAR")
    ...
    fear_greed_extreme_greed: int = Field(default=80, env="TACTICAL_FEAR_GREED_EXTREME_GREED")
...
class NotificationConfig(BaseSettings):
    ...
    notify_on_circuit_breaker: bool = Field(default=True, env="NOTIFY_ON_CIRCUIT_BREAKER")
...
class DatabaseConfig(BaseSettings):
    ...
    database_url: str = Field(
        default="sqlite:///./data/eternal_engine.db", 
        env="DATABASE_URL"
    )
    ...
    # SQLite fallback
    sqlite_url: str = Field(default="sqlite:///./data/eternal_engine.db", env="SQLITE_URL")
...
class DevelopmentConfig(BaseSettings):
    ...
    testnet_initial_balance_usdt: float = Field(
        default=100000.0, 
        env="TESTNET_INITIAL_BALANCE_USDT"
    )
    
    # Paper trading settings
    paper_initial_balance_usdt: float = Field(
        default=100000.0, 
        env="PAPER_INITIAL_BALANCE_USDT"
    )
    
    # Feature flags
    feature_flag_grid_trading: bool = Field(default=False, env="FEATURE_FLAG_GRID_TRADING")
    feature_flag_ml_predictions: bool = Field(default=False, env="FEATURE_FLAG_ML_PREDICTIONS")
...
class EternalEngineConfig:
    ...
    def validate_configuration(self) -> dict:
        ...
        if not api_secret or api_secret.startswith("your_"):
            issues.append(f"Missing or invalid API secret for {self.bybit.api_mode} mode")
        ...
        if not (self.circuit_breaker.level_1_threshold < 
