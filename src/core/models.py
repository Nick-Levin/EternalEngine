...
class Order(BaseModel):
    ...
    # Identification
    id: str = Field(
        default_factory=lambda: str(uuid4()), description="Internal order ID"
    )
    exchange_order_id: Optional[str] = Field(
        default=None, description="Exchange order ID"
    )
    ...
    filled_at: Optional[datetime] = Field(
        default=None, description="Fill completion time"
    )
    ...
    def price_required_for_limit(cls, v: Optional[Decimal], info) -> Optional[Decimal]:
        ...
        if order_type in (
            ...
            if v is None or v <= 0:
                raise ValueError(
                    f"Price required and must be positive for {order_type}"
                )
        return v
...
class Position(BaseModel):
    ...
    opened_at: datetime = Field(
        default_factory=datetime.utcnow, description="Open time"
    )
    ...
    max_drawdown_pct: Optional[Decimal] = Field(
        default=None, description="Max drawdown %"
    )

    # Perpetual futures specific
    leverage: Decimal = Field(
        default=Decimal("1"), ge=1, description="Leverage (1.0 = spot)"
    )
    margin_type: Literal["isolated", "cross"] = Field(
        default="cross", description="Margin mode"
    )
    ...
    def update_from_fill(
        self, fill_price: Decimal, fill_amount: Decimal, side: OrderSide
    ) -> None:
...
class Trade(BaseModel):
    ...
    entry_time: datetime = Field(
        default_factory=datetime.utcnow, description="Entry time"
    )
    exit_time: Optional[datetime] = Field(default=None, description="Exit time")

    # PnL
    realized_pnl: Decimal = Field(
        default=Decimal("0"), description="Gross realized PnL"
    )
    realized_pnl_pct: Decimal = Field(
        default=Decimal("0"), description="PnL percentage"
    )
...
class TradingSignal(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="Signal time"
    )
...
class RiskCheck(BaseModel):
    ...
    Attributes:
        ...
        risk_level: Risk level of the check (normal, warning, critical)
    ...
    risk_level: str = Field(
        default="normal", description="Risk level (normal, warning, critical)"
    )

    # Adjusted parameters
    max_position_size: Optional[Decimal] = Field(
        default=None, description="Approved size"
    )
    approved_leverage: Optional[Decimal] = Field(
        default=None, description="Approved leverage"
    )

    # Circuit breaker state
    circuit_breaker_level: CircuitBreakerLevel = Field(
        default=CircuitBreakerLevel.NONE, description="Active circuit breaker level"
    )

    # Adjusted risk levels
    adjusted_stop_loss: Optional[Decimal] = Field(
        default=None, description="Adjusted stop"
    )
    adjusted_take_profit: Optional[Decimal] = Field(
        default=None, description="Adjusted TP"
    )
...
class Portfolio(BaseModel):
    ...
    positions: Dict[str, Position] = Field(
        default_factory=dict, description="Positions"
    )
    ...
    daily_starting_balance: Decimal = Field(
        default=Decimal("0"), description="Day start balance"
    )
    weekly_starting_balance: Decimal = Field(
        default=Decimal("0"), description="Week start balance"
    )
    monthly_starting_balance: Decimal = Field(
        default=Decimal("0"), description="Month start balance"
    )
    yearly_starting_balance: Decimal = Field(
        default=Decimal("0"), description="Year start balance"
    )

    # Drawdown tracking
    all_time_high: Decimal = Field(default=Decimal("0"), description="Portfolio ATH")
    max_drawdown_pct: Decimal = Field(
        default=Decimal("0"), description="Max drawdown %"
    )

    # Engine allocations
    engine_allocations: Dict[EngineType, Decimal] = Field(
        default_factory=dict, description="Capital allocation per engine"
    )
    engine_values: Dict[EngineType, Decimal] = Field(
        default_factory=dict, description="Current value per engine"
    )
    ...
    def update_ath(self) -> None:
        ...
        if self.all_time_high > 0:
            self.max_drawdown_pct = (
                (self.all_time_high - self.total_balance) / self.all_time_high
            ) * 100
...
class EngineState(BaseModel):
    ...
    pause_until: Optional[datetime] = Field(
        default=None, description="Auto-resume time"
    )

    # Financial state
    current_allocation_pct: Decimal = Field(
        default=Decimal("0"), description="Allocation %"
    )
    ...
    # Circuit breaker
    circuit_breaker_level: CircuitBreakerLevel = Field(
        default=CircuitBreakerLevel.NONE, description="Active circuit breaker"
    )
    ...
    def can_trade(self) -> bool:
        ...
        if not self.is_active or self.is_paused:
            return False
        if self.circuit_breaker_level in (
    ...
    def record_trade(self, trade: Trade) -> None:
        ...
        if trade.is_profitable:
            ...
            self.metadata["gross_profit"] = (
                self.metadata.get("gross_profit", Decimal("0")) + trade.net_pnl
            )
        else:
            self.losing_trades += 1
            self.metadata["gross_loss"] = (
                self.metadata.get("gross_loss", Decimal("0")) + trade.net_pnl
            )
    ...
    def pause(self, reason: str, duration_seconds: Optional[int] = None) -> None:
        ...
        if duration_seconds:
            self.pause_until = datetime.utcnow() + __import__("datetime").timedelta(
...
def create_market_order(
    symbol: str, side: OrderSide, amount: Decimal, **kwargs
) -> Order:
    ...
    """
    return Order(
        symbol=symbol, side=side, order_type=OrderType.MARKET, amount=amount, **kwargs
    )


def create_limit_order(
    symbol: str, side: OrderSide, amount: Decimal, price: Decimal, **kwargs
) -> Order:
