...
class FundingEngine(BaseEngine):
    ...
    def __init__(
        ...
        self.delta_exposure: Dict[str, Decimal] = {a: Decimal("0") for a in self.funding_config.assets}
        ...
        for asset in self.funding_config.assets:
            ...
            if has_position:
                ...
                if exit_signal:
                    ...
                    rebalance_signal = self._check_rebalance_needed(asset, spot_price, perp_price)
                    if rebalance_signal:
                        signals.append(rebalance_signal)
            else:
                # Check entry conditions
                if self._check_entry_conditions(asset, basis):
                    entry_signals = self._create_entry_signals(asset, spot_price, perp_price)
    ...
    def _check_rebalance_needed(
        ...
        if delta_deviation > self.funding_config.rebalance_threshold_pct:
            return self._create_rebalance_signal(asset, spot_notional, perp_notional, delta)
    ...
    def _create_entry_signals(
        ...
        position_size = asset_allocation * Decimal("0.5")  # Use 50% of allocation per asset
    ...
    def _create_exit_signal(self, asset: str, reason: str, details: str) -> TradingSignal:
    ...
    async def on_order_filled(
        ...
        # Update delta
        spot_notional = position['spot_size'] * price if is_spot else position['spot_size'] * position.get('entry_spot_price', price)
        perp_notional = position['perp_size'] * price if not is_spot else position['perp_size'] * position.get('entry_perp_price', price)
